...
<html class="no-js" lang="en">
 ...
 <?php include_once '../sastrane.php' ;    ?>
   ...
   <div class="large-10 medium-8 columns centrirano">
  ...
  <tbody>
			...
			$s=($stranica-1)*9;
				...
				//print_r($_GET);
				$izraz=$veza -> prepare("select po.id as id,po.posiljatelj as posilja,po.vrijeme as vrijeme,
				po.procitano as status,usa.username as posiljatelj,usb.username as primatelj from poruka po 
				inner join user usa on po.posiljatelj=usa.id 
				inner join user usb on po.primatelj=usb.id 
				where po.primatelj=:id order by UNIX_TIMESTAMP(vrijeme) desc
					limit $s, 9");
				$izraz->execute($_GET);
				...
				<tr>
				 ...
				 <td><?php echo $mess->posiljatelj;?> </td>
				 <td><?php echo $mess->vrijeme;?> </td>
				  <td><a href="poruka.php<?php if($mess->status==0){echo "?a=123&id=".$mess->id."&pos=".$mess->posiljatelj;}else{echo "?id=".$mess->id."&pos=".$mess->posiljatelj;}?>">Pročitaj </a></td>
